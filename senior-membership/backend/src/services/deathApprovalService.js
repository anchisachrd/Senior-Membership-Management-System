...
export const checkCommitteesVote = async (
  reportId,
  committeeId,
  status,
  comment
) => {
  await deathApprovalModel.updateCommitteeApproval(
    reportId,
    committeeId,
    status,
    comment
  ...
  const approvals = await deathApprovalModel.getDeathApprovalStatusByReportId(
    reportId
  );

  const pending = approvals.some(
    (da) =>
      da.approval_status === "รอการพิจารณา" ||
      da.approval_status === "รอการแก้ไข"
  );
  if (pending) {
    return;
  }
  ...
  if (finalStatus === "อนุมัติ") {
    await deathReportModel.updateSentToHeir(true, reportId);
  } else if (finalStatus === "ไม่อนุมัติ") {
    await deathReportModel.updateSentToHeir(false, reportId);
  }
};
...
export const getFinalDeathApproval = async (reportId) => {
  try {
    ...
    const approvalRows = await deathApprovalModel.getCommitteeApproval(
      reportId
    );
